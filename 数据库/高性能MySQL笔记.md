...
读多写少的业务场景，**如果不介意MyISAM崩溃恢复问题**，选用MyISAM引擎是合适的（MyISAM**只将数据写到内存中**，然后等待操作系统**定期将数据刷出到磁盘上**，带来的问题是不能保证将数据安全地写入磁盘中）。

“MyISAM比InnoDB快”，不是绝对的。

“在很多我们已知的场景中，InnoDB的速度都可以让MyISAM望尘莫及，尤其是使用到**聚簇索引**，或者需要访问的数据都可以放入内存的应用。”



最好还是选用InnoDB，随着应用压力的上升，锁争用和崩溃后的数据丢失问题也会随之而来。
...
B+树，非叶子节点只存放索引的键，通过键去查找叶子节点，叶子节点中存放指向数据的指针（聚簇索引，索引和数据存放在一起）。
...
* 短索引（前缀索引）
  ...
  当字段很长，但是，前几个字符的组合基本上不一致，就可以用前缀索引（只使用前几个字符做索引）
...
* 如果查询中**有某个列的范围查询，则其右边所有列都无法使用索引优化查找**。
  ...
  所以在联合索引中要谨慎使用范围查找，一旦索引中的某个字段使用了范围查找，后面字段的索引就失效了。
  
  从B+数的结构可以看出来，每一列的索引和其他列的索引是有断层的（先按字段1排序，再按字段2排序，再按字段3排序。如果其中某个字段设置了范围查找，那么就只能从前两个字段排序查找出来的内容中进行遍历，查找满足第三个字段条件的数数据）
  
  ![image-20210602110513765](高性能MySQL笔记.assets/image-20210602110513765.png)

### 3.1.2 哈希索引(hash index)

哈希索引（Hash Index）基于哈希表实现，只有**精确匹配**索引所有列的查询才有效。对于**每一行数据**，存储引擎会**对所有索引列计算一个哈希码**（Hash Code），哈希码是一个比较小的值，并且不同键值对应的哈希码不同。
...
<img src=".\高性能MySQL笔记.assets\image-20201215194812804.png" alt="image-20201215194812804" style="zoom:80%;" />
